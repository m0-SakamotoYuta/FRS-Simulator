...
# FEM接触解析ソルバー
try:
	from fem_contact_solver_1 import (
...
class MainMenuGUI(tk.Tk):
	...
	def _create_cartilage_separation_tab(self) -> None:
		...
		vis_frame.grid(row=5, column=0, sticky="nsew", padx=12, pady=(0, 8))
	...
	def _create_fem_tab(self) -> None:
		...
		if not _HAS_FEM:
			ttk.Label(container, text="FEM解析モジュール (fem_contact_solver_1.py) が見つかりません。\n同じディレクトリに配置してください。",
		...
		ttk.Entry(mesh_frame, textvariable=self.fem_prox_bone_path, width=60).grid(row=1, column=1, sticky="ew", padx=4, pady=2)
		...
		ttk.Entry(mesh_frame, textvariable=self.fem_prox_cartilage_path, width=60).grid(row=2, column=1, sticky="ew", padx=4, pady=2)
		ttk.Button(mesh_frame, text="参照...", width=8,
		...
		ttk.Entry(mesh_frame, textvariable=self.fem_dist_bone_path, width=60).grid(row=4, column=1, sticky="ew", padx=4, pady=2)
		ttk.Button(mesh_frame, text="参照...", width=8,
		...
		ttk.Entry(mesh_frame, textvariable=self.fem_dist_cartilage_path, width=60).grid(row=5, column=1, sticky="ew", padx=4, pady=2)
		ttk.Button(mesh_frame, text="参照...", width=8,
	...
	def _create_pos_checker_tab(self) -> None:
		...
		ttk.Entry(model_frame, textvariable=self.pos_check_model_path).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
		...
		ttk.Entry(pp_frame, textvariable=self.pos_check_pp_pv_path).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
		...
		ttk.Entry(pp_frame, textvariable=self.pos_check_pp_fm_path).grid(row=2, column=1, padx=5, pady=5, sticky="ew")
		...
		ttk.Entry(comp_frame, textvariable=self.pos_check_mat1_path).grid(row=0, column=1, padx=5, pady=5, sticky="ew")
		...
		ttk.Entry(comp_frame, textvariable=self.pos_check_mat2_path).grid(row=1, column=1, padx=5, pady=5, sticky="ew")
	...
	def _setup_fonts(self) -> None:
		...
		self.ui_font_family = pick([
			"Yu Gothic UI", "Meiryo UI", "Segoe UI", "メイリオ", "Yu Gothic", "MS UI Gothic", "MS Gothic", "Arial"
		])
		self.mono_font_family = pick([
	...
	def on_cs_execute(self) -> None:
		...
		try:
			...
			# --- Phase 3: ブーリアン差分（引き算）---
			print("\n--- Phase 3: 軟骨領域の抽出 (モデル1 − モデル2 = 軟骨＋関節唇) ---")

			cartilage_mesh = None

			# 方法1: PyVista boolean_difference（VTKベース）
			print("方法1: VTKブーリアン差分を試行中...")
			try:
				cartilage_mesh = model1_whole.boolean_difference(model2_transformed)
				if cartilage_mesh is not None and cartilage_mesh.n_cells > 0:
					print(f"  ✓ VTKブーリアン差分 成功: {cartilage_mesh.n_points}点, {cartilage_mesh.n_cells}面")
				else:
					cartilage_mesh = None
					print("  ✗ VTKブーリアン差分: 結果が空")
			except Exception as e_bool:
				print(f"  ✗ VTKブーリアン差分 失敗: {e_bool}")
				cartilage_mesh = None

			# 方法2: 距離ベースの差分（フォールバック）
			if cartilage_mesh is None:
				print("\n方法2: 距離ベースの軟骨抽出を実行中...")
				print("  モデル1の各頂点からモデル2への最短距離を計算し、")
				print(f"  距離が閾値({self.cs_distance_threshold.get():.2f}mm)を超える領域を軟骨として抽出します。")
				try:
					from scipy.spatial import cKDTree

					# モデル2の表面点でKD-Treeを構築
					m2_surface = model2_transformed.extract_surface()
					tree = cKDTree(m2_surface.points)

					# モデル1の各頂点からモデル2への最短距離
					m1_surface = model1_whole.extract_surface()
					distances, _ = tree.query(m1_surface.points)

					threshold = self.cs_distance_threshold.get()

					# 距離が閾値を超える頂点（＝モデル2に含まれない＝軟骨）
					mask = distances > threshold
					n_selected = np.sum(mask)
					print(f"  距離 > {threshold:.2f}mm の頂点: {n_selected}/{m1_surface.n_points} ({100*n_selected/max(1,m1_surface.n_points):.1f}%)")
					...
					if n_selected > 0:
						...
						if cartilage_mesh.n_cells > 0:
							...
							try:
								...
								pass  # クリーンアップ失敗は致命的でない

							print(f"  ✓ 距離ベース抽出 成功: {cartilage_mesh.n_points}点, {cartilage_mesh.n_cells}面")
						else:
							cartilage_mesh = None
							print("  ✗ 距離ベース抽出: セル抽出結果が空")
					else:
						print("  ✗ 距離ベース抽出: 閾値を超える頂点なし（閾値を小さくしてみてください）")
			...
			if cartilage_mesh is None or cartilage_mesh.n_cells == 0:
				messagebox.showerror("エラー",
					"軟骨領域の抽出に失敗しました。\n\n"
					"考えられる対処:\n"
					"・軟骨分離タブの「距離閾値」を小さくする（現在: {:.2f}mm）\n"
					"・位置合わせを改善（RANSAC/ICPパラメータを調整）\n"
					"・モデルの品質を確認（穴、反転法線など）".format(self.cs_distance_threshold.get()))
				return
	...
	def _show_precompute_dialog(self, total_frames):
		...
		# プログレスダイアログを作成
		progress_window = tk.Toplevel(self)
		progress_window.title("ヒートマップ事前計算")
		progress_window.geometry("550x700")
		progress_window.resizable(False, False)
	...
	def on_animate(self) -> None:
		...
		try:
			...
			# --- オーバーラップ体積の事前計算 ---
			# 事前計算ダイアログを表示
			progress_window, update_progress, cancel_var, options_dict, start_var, skip_var = self._show_precompute_dialog(len(transform_data))
			...
			# --- FEM接触解析ボタン（軟骨モデルが読み込まれている場合のみ表示）---
			if _HAS_FEM and prox_cartilage_mesh is not None and dist_cartilage_mesh is not None:
				def run_fem_current_frame():
					"""現在フレームの姿勢でFEM接触解析を実行"""
					was_paused = is_paused[0]
					is_paused[0] = True
					pause_button.config(text="再生")
					self._run_fem_on_current_frame(
						prox_cartilage_mesh, dist_cartilage_mesh,
						prox_bone_mesh=prox_mesh, dist_bone_mesh=dist_mesh,
					)
					if not was_paused:
						toggle_pause()

				fem_anim_btn = ttk.Button(button_frame, text="FEM解析", width=12, command=run_fem_current_frame)
...
def main():
	"""メインエントリーポイント"""
	# Open3DによるDPI設定の強制変更（ウィンドウ縮小現象）への対策
	# アプリケーション起動時から高DPI対応(System DPI Aware)としてマークすることで、
	# 途中で設定が切り替わるのを防ぎ、サイズ変化を抑制する。
	# ※これにより起動時からUIサイズがOSのスケーリング設定に合わせて調整される（通常は小さく/精細になる）
	try:
		import ctypes
		# SetProcessDpiAwareness: 0=Unaware, 1=System DPI Aware, 2=Per Monitor DPI Aware
		# Tkinterには 1 が比較的安全
