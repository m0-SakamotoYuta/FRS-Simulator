...
class MainMenuGUI(_BaseWindow):
	...
	def on_ankle_animate(self) -> None:
		...
		# --- マーカー間距離 (キャリブを通していない生の検出結果) ---
		# 骨間距離 = ①ArUco検出 × ②キャリブ の結果なので、ズレていても原因が分からない。
		# マーカー間距離は ① だけで決まるので、実物をメジャーで測った値と直接比較でき、
		# ①②のどちらが原因かをモード切替なしで切り分けられる。
		marker_pos = {}
		for (idx, name, mesh_L, Ts) in bones_data:
			bone = self.ankle_bones[idx] if idx < len(self.ankle_bones) else {}
			aid = int(bone.get("aruco_id", -1))
			...
			try:
				...
				if len(vi) == 0:
					continue
				marker_pos[idx] = (name, poses_C[vi[0]][:3, 3])
			except Exception:
				pass
		if len(marker_pos) >= 2:
			keys = list(marker_pos.keys())
			for a in range(len(keys)):
				for b in range(a + 1, len(keys)):
					na, pa = marker_pos[keys[a]]
					nb, pb = marker_pos[keys[b]]
					dm = float(np.linalg.norm(pa - pb))
					# 同じ骨対の「骨間距離」も引いてきて並べる
					db = None
					try:
					...
					print(f"  マーカー間距離 [{na} ↔ {nb}] @ t=0: {dm:.1f} mm "
					      f"← ①ArUco検出のみで決まる。実物をメジャーで測った値と比べること")
					if db is not None:
						print(f"      (参考) 骨間距離 = {db:.1f} mm ← ①×②キャリブ の結果")
						if dm < 300 and db > 300:
							print("      [切り分け] マーカー間は妥当なのに骨間が異常 "
							      "→ ②キャリブ (marker_to_bone_T) を疑う")
						elif dm > 300:
							print("      [切り分け] マーカー間距離そのものが異常 "
							      "→ ①ArUco検出を疑う (深度スケール・マーカー実寸・検出率)")
		print("=" * 70)
		...
		# 【重要】.db3 のタイムスタンプは絶対時刻(エポック秒 ≈ 1.79e9)なので、
		# 先頭を 0 秒に正規化してから使う。再生クロックは 0 から数え始めるため、
		# 絶対時刻のままだと「frame_times[i] <= actual_time」が永遠に成立せず、
		# 再生ボタンを押してもフレームが進まない。
		ts_cache = cache.get("timestamps", None)
		if ts_cache is not None and len(ts_cache) >= N:
			raw = [float(ts_cache[t]) for t in range(N)]
			t0 = raw[0]
			frame_times = [v - t0 for v in raw]
			# 単調増加でない/異常な場合は等間隔にフォールバック
			if N > 1 and (frame_times[-1] <= 0 or not all(
					frame_times[i] <= frame_times[i + 1] + 1e-9 for i in range(N - 1))):
				print("[ankle animate] タイムスタンプが単調増加でないため等間隔に置換します")
