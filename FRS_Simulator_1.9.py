...
class MainMenuGUI(_BaseWindow):
	...
	def _show_precompute_dialog(self, total_frames, has_cartilage=False):
		...
		Args:
			total_frames: 総フレーム数
			has_cartilage: 軟骨モデルが読み込まれているかどうか
		...
		fem_check = ttk.Checkbutton(
			fem_frame,
			text="FEM接触解析を事前計算する（軟骨モデルが必要・計算に時間がかかります）",
		...
		if not _HAS_FEM:
			fem_check_note = ttk.Label(fem_frame, text="FEMモジュール (fem_contact_solver_1.py) が見つかりません")
			fem_check_note.pack(anchor=tk.W, padx=20, pady=2)
		elif not has_cartilage:
			fem_check_note = ttk.Label(fem_frame, text="軟骨モデルが読み込まれていません")
			fem_check_note.pack(anchor=tk.W, padx=20, pady=2)
	...
	def on_animate(self) -> None:
		...
		try:
			...
			progress_window, update_progress, cancel_var, options_dict, start_var, skip_var = self._show_precompute_dialog(len(transform_data), has_cartilage=has_cartilage)
			...
			if options_dict.get('enable_fem_precompute') and options_dict['enable_fem_precompute'].get():
				if prox_cartilage_mesh is not None and dist_cartilage_for_anim is not None and _HAS_FEM:
					print(f"[FEM事前計算] {len(transform_data)}フレーム分のFEM解析を実行します...")
					...
					try:
						...
						for i, tf_info in enumerate(transform_data):
							...
							try:
								...
								dist_origin_initial = np.array([0.0, 0.0, 0.0])
								...
								dist_cart_frame = dist_cartilage_for_anim.copy()
								dist_cart_frame.points = cart_trans

								prox_cart_frame = prox_cartilage_mesh.copy()

								# FEM解析を実行
								solver = FEMContactSolver(material=material, contact=contact, verbose=False)
								results = solver.analyze(
									prox_cart_frame, dist_cart_frame,
									boundary_mode=self.fem_boundary_mode.get(),
									max_nodes=self.fem_max_nodes.get(),
								)

								# 可視化用メッシュを作成
								mesh_vis = prox_cart_frame.copy()
								if results.n_nodes == mesh_vis.n_points:
									apply_fem_results_to_mesh(mesh_vis, results)
								else:
									mesh_vis = pv.PolyData(solver._nodes, prox_cart_frame.faces)
									apply_fem_results_to_mesh(mesh_vis, results)

								fem_precomputed.append((mesh_vis, results))
								...
								fem_precomputed.append(None)

						print(f"[FEM事前計算] 完了: {sum(1 for x in fem_precomputed if x is not None)}/{len(transform_data)}フレーム成功")

					except Exception as e:
						print(f"[FEM事前計算] 初期化エラー: {e}")
						messagebox.showerror("FEM事前計算エラー", f"FEM事前計算中にエラーが発生しました:\n{e}")
						fem_precomputed = []
			...
			# --- FEM結果メッシュの初期化 ---
			print(f"[初期化] FEM結果メッシュの初期化を開始...")
			fem_mesh = None
			fem_mesh_actor = None
			if fem_precomputed and any(x is not None for x in fem_precomputed):
				try:
					mesh_vis, results = fem_precomputed[0]
					scalar_name = self.fem_scalar_name.get()
					cmap = self.fem_cmap.get()
					fem_mesh = mesh_vis.copy()
					...
					fem_mesh_actor = anim_plotter.add_mesh(
						fem_mesh, scalars=scalar_name, cmap=cmap,
						show_edges=False, opacity=0.9,
						scalar_bar_args={'title': label, 'color': 'black'},
						label='FEM結果'
					)
					fem_mesh_actor.SetVisibility(False)  # デフォルトで非表示
					print(f"[初期化] FEM結果メッシュを初期化: {fem_mesh.n_points}点")
				except Exception as e:
					print(f"[警告] FEM結果メッシュの初期化失敗: {e}")
					fem_mesh = None
					fem_mesh_actor = None
			else:
				print(f"[初期化] FEM事前計算データがないため、FEM結果メッシュは作成されません")
			...
			# FEM結果の表示/非表示
			def toggle_fem(state):
				if fem_mesh_actor is not None and fem_precomputed:
					fem_mesh_actor.SetVisibility(state)
				else:
					if fem_mesh_actor is not None:
						fem_mesh_actor.SetVisibility(False)
			...
			if heatmap_enabled:
				...
				# チェックボックスとラベル - FEM結果（事前計算されている場合のみ表示）
				if fem_precomputed and any(x is not None for x in fem_precomputed):
					anim_plotter.add_text("FEM", position=(10, 190), font_size=10, color="black")
					anim_plotter.add_checkbox_button_widget(
						toggle_fem,
			...
			def show_frame(frame_idx, force_render=False):
				...
				try:
					...
					# FEM結果メッシュの更新
					if fem_precomputed and frame_idx < len(fem_precomputed):
						fem_data = fem_precomputed[frame_idx]
						if fem_data is not None and fem_mesh is not None and fem_mesh_actor is not None:
							try:
								mesh_vis, results = fem_data
								scalar_name = self.fem_scalar_name.get()
								fem_mesh.points = mesh_vis.points.copy()
								if scalar_name in mesh_vis.array_names:
									fem_mesh[scalar_name] = mesh_vis[scalar_name].copy()
							except Exception as e:
								if frame_idx == 0:
									print(f"[警告] FEM結果メッシュ更新失敗: {e}")
