...
class MainMenuGUI(tk.Tk):
	...
	def _build_transform_matrix(self, rz: float, rx: float, ry: float, 
		...
		Args:
			rz, rx, ry: Z, X, Y軸周りの回転角度 (度)
			ml: Medial/Lateral変位 (Z軸方向)
			ap: Anterior/Posterior変位 (X軸方向)
			pd: Proximal/Distal変位 (Y軸方向)
		...
		# 4x4同次変換行列を構築
		# 変位ベクトル: [X, Y, Z] = [AP, PD, ML]
		T = np.eye(4)
		T[:3, :3] = R
		T[:3, 3] = [ap, pd, ml]  # X=AP, Y=PD, Z=ML
	...
	def _show_transform_matrices(self, data):
		...
		# ヘッダー行を表示
		header = f"{'Time':>8}  {'FE':>8}  {'ML':>8}  {'VV':>8}  {'AP':>8}  {'IE':>8}  {'PD':>8}\n"
		...
		for item in data:
			time = item["time"]
			angles = item["angles"]  # [FE, VV, IE]
			trans = item["translations"]  # [ML, AP, PD]
			
			# Time, FE, ML, VV, AP, IE, PD の順に表示
			line = f"{time:8.3f}  {angles[0]:8.3f}  {trans[0]:8.3f}  {angles[1]:8.3f}  {trans[1]:8.3f}  {angles[2]:8.3f}  {trans[2]:8.3f}\n"
	...
	def _show_precompute_dialog(self, total_frames):
		...
		progress_window.geometry("550x550")
		progress_window.resizable(False, False)
	...
	def on_fitting_execute(self) -> None:
		...
		try:
			...
			# RANSACパラメータを取得
			voxel_size = self.ransac_distance_threshold.get()  # ボクセルサイズとして距離閾値を使用
			max_iterations = self.ransac_max_iterations.get()
			confidence = int(self.ransac_confidence.get() * 1000)  # 0.99 → 990
			
			print(f"RANSACパラメータ: voxel_size={voxel_size}, max_iterations={max_iterations}, confidence={confidence}")
	...
	def _show_fitting_result_menu(self, child_model_transformed, child_to_parent_transform, child_model_path, load_mesh_o3d):
		...
		x = (menu_window.winfo_screenwidth() // 2) - (600 // 2)
		y = (menu_window.winfo_screenheight() // 2) - (750 // 2)
	...
	def on_animate(self) -> None:
		...
		try:
			...
			control_window.geometry("850x240")
